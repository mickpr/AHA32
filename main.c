...
//  LCD_Clear();
  ...
  lsb = owire_read_byte();      // odczytanie mlodszych bit�w temperatury
  msb = owire_read_byte();      // odczytanie starszych bit�w temperatury
...
//					{	uint8_t j=0;
//					 	while (j<i) {
//					 		uart_printf("%c",pchr[j]); // drukuj lini� komend (passwd=dupa+pawiana (urlencode/decode))
//					 	 	j++; }
//					}
					i=i-7;
...
//				uart_printf("\n\r");
				...
				if (cmd==-1){
					plen=http401unauthorized();
					//plen=fill_tcp_data_p(buf,0,PSTR("HTTP/1.0 401 Unauthorized\r\nContent-Type: text/html\r\n\r\n<h1>401 Nieautoryzowany dost�p.</h1>"));
					goto SENDTCP;
				}
